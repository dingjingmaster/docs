...
/dev/modem不应该用于既支持拨号又支持拨号的调制解调器，因为它往往会导致锁定文件问题。如果存在，/dev/modem应该指向适当的主TTY设备(不建议使用备用调出设备)。

对于SCSI设备，/dev/tape和/dev/cdrom应该指向熟设备(分别为`/dev/st*`和`/dev/sr*`)，而/dev/cdwriter和/dev/scanner应该指向适当的通用SCSI设备(`/dev/sg*`)。
