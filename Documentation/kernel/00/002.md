...
该列表是Linux设备列表，是为Linux操作系统分配的设备号和/dev目录节点的官方注册表。

本文档通过引用包含在文件系统层次标准(FHS)中。可从https://www.pathname.com/fhs/获得FHS。

标记为(68k/Amiga)的分配仅适用于Amiga平台上的Linux/68k。标记为(68k/Atari)的分配仅适用于Atari平台上的Linux/68k。
...
本节详细介绍/dev目录中应该存在或可能存在的其他条目。符号链接最好使用此处所示的相同形式(绝对或相对)。根据首选的链接类型，链接分为“硬”链接或“符号”链接;如果可能的话，应该使用指定的链接类型。
...
|`/dev/stdin`|`fd/0`|symbolic| File descriptors|
|`/dev/stdout`|`fd/1`|symbolic| File descriptors|
|`/dev/stderr`|`fd/2`|symbolic| File descriptors|
|`/dev/nfsd`|`socksys`|symbolic| Required by iBCS-2|
|`/dev/X0R`|`null`|symbolic| Required by iBCS-2|

> 注:/dev/X0R为<字母X>-<数字0>-<字母R>。
...
|`/dev/i2o*`|`/dev/i2o0/*`|symbolic|Backward compatibility|
|`/dev/scd?`|`sr?`|hard|Alternate SCSI CD-ROM name|

### 本地定义的链接

根据系统的配置，可以在本地建立以下链接。这仅仅是对现有做法的一个列表，并不构成建议。但是，如果它们存在，它们应该具有以下用途。

|||||
|-----|-----|-----|-----|
|`/dev/mouse`||symbolic|Current mouse device|
|`/dev/tape`||symbolic|Current tape device|
|`/dev/cdrom`||symbolic||
|`/dev/cdwriter`||symbolic||
|`/dev/scanner`||symbolic|Current scanner device|
|`/dev/modem`||symbolic|Current dialout device|
|`/dev/root`||symbolic|Current root filesystem|
|`/dev/swap`||symbolic|Current swap device|

`/dev/modem`不应该用于既支持拨号又支持拨号的调制解调器，因为它往往会导致锁定文件问题。如果存在，`/dev/modem`应该指向适当的主TTY设备(不建议使用备用调出设备)。

对于SCSI设备，`/dev/tape`和`/dev/cdrom`应该指向热设备(分别为`/dev/st*`和`/dev/sr*`)，而`/dev/cdwriter`和`/dev/scanner`应该指向适当的通用SCSI设备(`/dev/sg*`)。

`/dev/mouse`可以指向一个主要的串行TTY设备，一个硬件鼠标设备，或者一个用于鼠标驱动程序的套接字(例如`/dev/gpmdata`)。

### sockets 和 pipes

非瞬态套接字和命名管道可能存在于/dev中。常见的分录有:
...
以下名称是为挂载/dev下的特殊文件系统保留的。这些特殊的文件系统提供了标准设备节点无法提供的内核接口。
...
终端或TTY设备是一类特殊的字符设备。终端设备是任何可以作为会话控制终端的设备;这包括虚拟控制台、串行端口和伪终端(pty)。

所有终端设备共享一组称为线路规则的通用功能;这些包括公共终端线路规则以及SLIP和PPP模式。

所有终端设备的命名都相似;介绍各种类型的tty的命名和使用方法。请注意，命名约定包含几个历史缺陷;其中一些是特定于Linux的，一些是从其他系统继承来的，还有一些反映了Linux超越了借用的惯例。

设备名称中的散列标记(#)在这里用于表示没有前导零的十进制数。

### 虚拟控制台和控制台设备

虚拟控制台是系统视频监视器上的全屏终端显示器。虚拟控制台命名为`/dev/tty#`，编号从`/dev/tty1`开始;`/dev/tty0`为当前虚拟控制台。`/dev/tty0`是应该用于访问那些框架缓冲设备(`/dev/fb*`)不适用的系统显卡的设备。不要将`/dev/console`用于此目的。

控制台设备`/dev/console`是应该向其发送系统消息的设备，并且应该允许在单用户模式下登录。从Linux 2.1.71开始，`/dev/console`由内核管理;对于以前的版本，它应该是一个符号链接，指向`/dev/tty0`，一个特定的虚拟控制台，如`/dev/tty1`，或一个串行端口主设备(`tty*`，而不是`cu*`)，这取决于系统的配置。

### 串行端口

串行端口是`RS-232`串行端口和任何模拟串行端口的设备，无论是在硬件(如内部调制解调器)还是在软件(如ISDN驱动程序)中。在Linux下，每个串行端口都有两个设备名称，一个是主设备或调用设备，另一个是备用设备或调用设备。每种装置都用不同的字母表示。对于任意字母X，设备的名称分别为`/dev/ttyX#`和`/dev/cux#`;由于历史原因，`/dev/tty#`和`/dev/tty#`对应于`/dev/cua#`和`/dev/cub#`。在未来，应该期望使用多个字母;对于“tty”设备(例如`/dev/ttyDP#`)，所有字母都是大写的，对于“cu”设备(例如`/dev/cudp#`)，所有字母都是小写的。

名称`/dev/ttyQ#`和`/dev/cuq#`保留给本地使用。

备用设备提供基于内核的排除，默认值与主设备略有不同。它们的主要目的是允许使用串行端口与程序没有固有的或破碎的串行端口支持。不推荐使用它们，它们可能会从未来的Linux版本中删除。

串口仲裁是通过使用名为`/var/lock/LCK..ttyX#`的锁文件提供的。锁文件的内容应该是锁进程的PID (ASCII码)。

通常的做法是安装指向串行端口的链接，例如`/dev/modem`。为了确保在存在这些链接时正确锁定，建议软件跟踪符号链接并锁定所有可能的名称;另外，建议在相应的备用设备上安装一个锁文件。为了避免死锁，建议按以下顺序获取锁，并按相反的顺序释放锁:
- 符号链接名称，如果有的话(`/var/lock/LCK..modem`)
- “tty”名称(`/var/lock/LCK..ttyS2`)
- 备用设备名称(`/var/lock/LCK..cua2`)

在嵌套符号链接的情况下，锁文件应该按照解析符号链接的顺序安装。
在任何情况下，应用程序都不应该在等待另一个锁被释放的同时持有一个锁。此外，尝试为相应的备用设备名创建锁文件的应用程序应该考虑在非串行端口TTY上使用的可能性，因为没有备用设备存在。

### Pseudoterminals(PTYs)

伪终端(pty)用于创建登录会话或为任意数据生成过程提供需要TTY线路规则(包括SLIP或PPP功能)的其他功能。每个PTY有一个主端，命名为`/dev/pty[p-za-e][0-9a-f]`，和一个从端，命名为`/dev/tty[p-za-e][0-9a-f]`。内核通过允许每个主端只打开一次来仲裁pty的使用。

一旦打开了主端，就可以像使用TTY设备一样使用相应的从端设备。主设备和从设备由内核连接，从而生成具有TTY功能的等效双向管道。

最新版本的Linux内核和GNU libc包含对PTY的System V/unix命名方案的支持，该方案为所有主服务器分配一个公共设备`/dev/ptmx`(打开它将自动给您一个以前未分配的PTY)，并为从服务器分配一个子目录`/dev/pts`;从服务器使用十进制整数(`/dev/pts/#`)命名。这消除了耗尽名称空间的问题，并使内核能够根据需要使用“devpts”文件系统自动为从服务器创建设备节点。
