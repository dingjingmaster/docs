...
以下是由`__setup()`，`early_param()`，`core_param()`和`module_param()`宏实现的内核参数的统一列表，按英文字典顺序排序，并提供描述。

内核解析参数从内核命令行的 `--` 开始；如果内核无法识别传入的参数，并且参数中不包含`.`，参数就会被传入到init中，如果参数中包含`=`符号，则会被传入到 init 的环境变量中，其它将会当作命令行参数传入到init。在 `--` 之后的所有内容都会被当作参数传入init。

模块参数可以通过两种方法传入：
1. 在命令行中使用`模块名.参数=值`这种方式传入
2. 通过modprobe命令行传入
例如：
```
# 内核命令行传入
...
内核中内置模块的参数需要在内核命令行上指定。modprobe查看内核命令行(/proc/cmdline)，并在加载模块时收集模块参数，因此内核命令行也可以用于可加载模块。

连字符(`-`)和下划线(`_`)在参数名中是等价的，所以以下两种写法是等价的:
```
...
一些内核参数接受一个cpu列表作为值，例如`isolcpus`, `nohz_full`,`irqaffinity`, `rcu_nocbs`。格式如下:
...
```
请注意，对于范围的特殊情况，可以将范围分成大小相等的组，每组使用该组开头的一些量:
```
...
其中最后一项表示cpu 100,101,125,126,150,151，…

数值“N”可以用来表示系统上最后一个CPU，即“`foo_cpus=16-N`”相当于32核系统上的“16-31”。

请记住，“N”是动态的，因此如果系统更改导致范围发生变化，例如CPU列表中的核数减少，那么N和使用N的任何范围也将发生变化。在小型4核系统上使用相同的方法，“16-N”变成了“16-3”，现在相同的引导输入将被标记为无效(因为start > end)。

“all”(忽略大小写)具有选择所有cpu的含义，因此“`nohz_full=all`”相当于“`nohz_full=0-N`”。

“N”和“all”的语义在`bitmap_*`级别上得到支持，并适用于`bitmap_parse()`。

命令"`modinfo -p ${modulename}`"显示可加载模块的所有参数的当前列表。可加载模块在被加载到运行的内核中之后，也会在`/sys/module/${modulename}/parameters/`中显示参数。其中一些参数可以在运行时通过命令`echo -n ${value} > /sys/module/${modulename}/parameters/${parm}`修改。

下面列出的参数只有在启用了某些内核构建选项并且存在相应的硬件时才有效。每个描述开头方括号内的文本说明了参数适用的限制:
```
...
用BOOT表示的参数实际上是由引导加载程序解释的，对内核没有直接意义。除非非常需要或与<the Linux/x86 boot Protocol>协调，否则不要修改引导加载程序参数的语法。

这里还没有记录特定于arch的内核参数。参见<AMD64 Specific Boot Options>。

请注意，下面列出的所有内核参数都是大小写敏感的，并且任何参数名称后面的=表示该参数将作为环境变量输入，而没有它表示它将作为内核参数出现，在系统启动后运行的程序可以通过`/proc/cmdline`读取。

内核参数的数量不受限制，但是完整命令行(参数包括空格等)的长度被限制为固定数量的字符。这个限制取决于体系结构，在`256`到`4096`个字符之间。它在文件。`/include/uapi/asm-generic/setup.h`中定义为`COMMAND_LINE_SIZE`。

最后，[KMG]后缀通常在一些内核参数值之后描述。这些'K'， 'M'和'G'字母代表二进制乘数'Kilo'， 'Mega'和'Giga'，分别等于`2^10`,`2^20`和`2^30`字节。这样的字母后缀也可以完全省略:
```
